...
function initPuertofolio() {
    ...
    var e = $("#project-gallery").swiper({
        ...
        onSlideChangeStart: function () {
            if ($(".pag2").hasClass("swiper-slide-visible")) 
                $(".swipe-info").animate({
                    opacity: .1
                }, 500);
            else 
    ...
    $(window).bind("resize", function () {
        $(".inner img").center({transition: 300});
    });

    function initIsotope() {
        //  isotopes------------------
        if ($(".gallery-items").length) {
            var i = $(".gallery-items").isotope({singleMode: true, columnWidth: ".grid-sizer,.grid-sizer-second,.grid-sizer-three", itemSelector: ".gallery-item, .gallery-item-second, .gallery-item-three"});
    ...
    $(".sidebar").scrollToFixed({marginTop: 120, minWidth: 1024});
    $(".blog-holder").css({
    ...
    // if (window.addEventListener)     window.addEventListener('message',
    // onMessageReceived, false); else     window.attachEvent('onmessage',
    // onMessageReceived, false); function onMessageReceived(e) {     var data =
    // JSON.parse(e.data);     switch (data.event) {         case 'ready':   var
    // data = { method: 'setVolume',                 value: '0'             }; $f[0]
    //                 .contentWindow .postMessage(JSON.stringify(data), url);
    //       break;     } } IMPORTANT   INIT YOUR FUNCTIONS HERE ------------------

    $(".accordion").each(function () {
        var a = 2 * $(this).attr("data-name");
        $(this)
            .find(".accordion-inner:nth-child(" + a + ")")
            .show();
        $(this)
            .find(".accordion-inner:nth-child(" + a + ")")
            .prev()
    ...
    if (null == trueMobile) {
        var b = $(".folio-content").kinetic({
            filterTarget: function (a, b) {
                if (!/down|start/.test(b.type)) 
                    return !/area|a|input/i.test(a.tagName);
                }
            ,
            maxvelocity: 1290,
            moved: function () {
                $(".kinekt-wrapper").css({cursor: "-webkit-grabbing"});
            },
            stopped: function () {
                $(".kinekt-wrapper").css({cursor: "-webkit-grab"});
            }
        });
...
$(".shareSelector").socialShare({social: "facebook,twitter,google,pinterest,linkedin", whenSelect: true, selectContainer: ".shareSelector", blur: false});
function showShare() {
    shrcn.fadeIn();
...
$(document).on("contextmenu", '.gallery-item , .swiper-slide', function (a) {
    ...
    $("#body-copyright")
        .show(10)
        .css("top", a.screenY - 90)
        .css("left", a.screenX + 10)
        .removeClass("scale-bg5");
...
$(function () {
    $.templateCore({
        templateItem: "#wrapper", outDuration: 150, inDuration: 50, loadErrorMessage: "404 page not found.", // 404 Message text
